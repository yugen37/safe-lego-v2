  ...
  return (
    <div className="self-start max-lg:col-span-5 max-md:order-1 max-md:col-span-full lg:col-span-4">
      <div className="rounded-medium shadow-nav dark:bg-dark-200 bg-white p-2.5">
        <div className="dark:border-borderColour-dark dark:bg-dark-200 rounded border border-dashed border-gray-100 bg-white px-8 py-12 max-md:px-4">
          <div className="mb-12">
            ...
            <label className="relative block">
              ...
              <input
                className="border-borderColour font-jakartaeholder:font-jakarta placer:text-paragraph-light focus:border-primary dark:border-borderColour-dark dark:text-paragraph-light dark:focus:border-primary w-full rounded-[60px] border py-5 pr-5 pl-12 transition-all duration-300 focus:outline-none dark:bg-transparent"
                placeholder="Search..."
                type="text"
                value={search}
            ...
            <div className="[&>*:not(:last-child)]:border-borderColour dark:[&>*:not(:last-child)]:border-borderColour-dark [&>*:not(:last-child)]:border-b [&>*:not(:last-child)]:border-dashed">
              {uniqueCategories.map((category) => (
                <Link
                  ...
                  className={`group font-jakartare:bg-paragraph relative flex items-center justify-between py-5 text-lg font-medium before:absolute before:bottom-0 before:left-0 before:h-[1px] before:w-full before:origin-right before:scale-x-0 before:transition-transform before:duration-500 before:content-[''] before:hover:origin-left before:hover:scale-x-100 dark:before:bg-white ${
                    setActive == category.data.categories ? 'blogActive' : ''
                  } `}
                  key={category}>
                  {category.data.categories}
                  <FaAngleRight className="hidden group-[.blogActive]:block" />
                </Link>
              ))}
            </div>
          </div>
              ...
              {uniqueTags.map((item) => (
                <div key={item.data.tags} className={`${setActive == item.data.tags ? 'tagActive group' : ''}`}>
                  <Link
                    ...
                    className="btn-outline btn-sm group-[.tagActive]:bg-paragraph dark:group-[.tagActive]:bg-primary dark:group-[.tagActive]:text-paragraph group-[.tagActive]:text-white">
                    {item.data.tags}
                  </Link>
                </div>
              ))}
